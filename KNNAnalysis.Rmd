...
The KNN model can predict with a likelihood of 94% the chance that a county wears masks.


# Confusion Matrix

```{r}
knnresult <- knn(TrainingX, TestingX, TrainingY, 12)
table(TestingK, knnresult)
```


1040/1108 = 94.04 (reaffirms high classification rate)

# KNN with 10 fold CV

```{r}
cv <- knn.cv(TrainingX, TrainingY, 12)
table(TestingK, cv)
```
